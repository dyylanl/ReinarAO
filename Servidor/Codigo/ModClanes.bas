...
Public Sub EcharMember(UserIndex As Integer, ByVal rdata As String)
    ...
    If MemberIndex Then
        ...
        UserList(MemberIndex).GuildInfo.echadas = UserList(MemberIndex).GuildInfo.echadas + 1
        UserList(MemberIndex).GuildInfo.GuildPoints = UserList(MemberIndex).GuildInfo.GuildPoints - 25
        UserList(MemberIndex).GuildInfo.GuildName = ""
        Call UpdateUserChar(MemberIndex)
    Else
        Call SendData(ToGuildMembers, UserIndex, 0, "!G" & UserList(MemberIndex).name)
        Call SendData(ToIndex, MemberIndex, 0, "!H")
        Call WriteVar(App.path & "\CHARFILE\" & UCase(rdata) & ".CHR", "GUILD", "Echadas", UserList(MemberIndex).GuildInfo.echadas = UserList(MemberIndex).GuildInfo.echadas + 1)
        Call WriteVar(App.path & "\CHARFILE\" & UCase(rdata) & ".CHR", "GUILD", "GuildPts", UserList(MemberIndex).GuildInfo.GuildPoints = UserList(MemberIndex).GuildInfo.GuildPoints - 25)
        Call WriteVar(App.path & "\CHARFILE\" & UCase(rdata) & ".CHR", "GUILD", "GuildName", "")
    End If
...
Public Sub DenyRequest(UserIndex As Integer, ByVal rdata As String)
    ...
    If MemberIndex Then
        ...
        UserList(MemberIndex).GuildInfo.SolicitudesRechazadas = UserList(MemberIndex).GuildInfo.SolicitudesRechazadas - 1
    Else
...
Public Sub AcceptClanMember(UserIndex As Integer, ByVal rdata As String)
    ...
    If oGuild.Members.Count >= 20 Then
    ...
    If MemberIndex Then
        If Len(UserList(MemberIndex).GuildInfo.GuildName) > 0 Then
            ...
            Exit Sub
        End If
        If AgregarAClan(Soli.UserName, UserList(UserIndex).GuildInfo.GuildName) = True Then
            UserList(MemberIndex).GuildInfo.GuildName = UserList(UserIndex).GuildInfo.GuildName
            Call UpdateUserChar(MemberIndex)
        End If
    Else
        If Len(GetVar(App.path & "\CHARFILE\" & UCase$(Soli.UserName) & ".CHR", "GUILD", "GuildName")) > 0 Then
            Call SendData(ToIndex, UserIndex, 0, "1H")
            Exit Sub
        End If
        If AgregarAClan(Soli.UserName, UserList(UserIndex).GuildInfo.GuildName) = False Then Exit Sub
    End If
...
End Sub
Public Function FetchGuild(GuildName As String) As Object
    Dim k As Integer
...
Public Function CanCreateGuild(UserIndex As Integer) As Boolean
    ...
    If Not TieneObjetos(409, 1, UserIndex) Then
        Call SendData(ToIndex, UserIndex, 0, "||¡¡Debes tener una gema celeste para fundar clan!!" & FONTTYPE_INFO)
        Exit Function
    End If


    If UserList(UserIndex).Stats.UserSkills(Liderazgo) < 100 Then
        Call SendData(ToIndex, UserIndex, 0, "||Para fundar clan necesitas 100 puntos de Skills en liderazgo" & FONTTYPE_INFO)
        Exit Function
    End If
    ...
    If UserList(UserIndex).Stats.ELV < 70 Then
        Call SendData(ToIndex, UserIndex, 0, "||Para fundar clan necesitas ser nivel 70." & FONTTYPE_INFO)
        Exit Function
    End If
