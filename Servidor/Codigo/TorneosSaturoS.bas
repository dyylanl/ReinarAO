...
Sub Torneoauto_Cancela()
    ...
    Call SendData(ToAll, 0, 0, "||Torneo: Torneo Automatico cancelado por falta de participantes." & FONTTYPE_GUILD)
...
Sub Rondas_Cancela()
    ...
    Torneo_Activo = False
    Torneo_Esperando = False
    Call SendData(ToAll, 0, 0, "||Torneo: Torneo Automatico cancelado por Game Master" & FONTTYPE_GUILD)
    frmMain.Torneos.Enabled = True
...
Sub Rondas_UsuarioMuere(ByVal UserIndex As Integer, Optional Real As Boolean = True, Optional CambioMapa As Boolean = False)
    ...
    If (Torneo_Rondas = 1) Then
        ...
        Call SendData(ToAll, 0, 0, "||GANADOR DEL TORNEO: " & UserList(Torneo_Luchadores(LI1)).name & FONTTYPE_GUILD)
        Call SendData(ToAll, 0, 0, "||PREMIO AL GANADOR: 1 punto de torneo, 1 copa de oro y 1 puntos de canje (Si es noble se le entregará el doble de premios)." & FONTTYPE_GUILD)
        If UserList(Torneo_Luchadores(LI1)).flags.EsNoble = 1 Then
            UserList(Torneo_Luchadores(LI1)).Faccion.Quests = UserList(Torneo_Luchadores(LI1)).Faccion.Quests + 2
            UserList(Torneo_Luchadores(LI1)).Faccion.Torneos = UserList(Torneo_Luchadores(LI1)).Faccion.Torneos + 2
            UserList(Torneo_Luchadores(LI1)).Ranking.TorneosGanados = UserList(Torneo_Luchadores(LI1)).Ranking.TorneosGanados + 2
            Call GuardarRanking("Torneo", Torneo_Luchadores(LI1))
            Dim Superoro As Obj
            Superoro.Amount = 2
            Superoro.OBJIndex = 824
            Call MeterItemEnInventario(Torneo_Luchadores(LI1), Superoro)
            UserList(Torneo_Luchadores(LI1)).flags.automatico = False
            Call SendUserStatsBox(Torneo_Luchadores(LI1))
            Torneo_Activo = False
            frmMain.Torneos.Enabled = True
        Else
            UserList(Torneo_Luchadores(LI1)).Faccion.Quests = UserList(Torneo_Luchadores(LI1)).Faccion.Quests + 1
            UserList(Torneo_Luchadores(LI1)).Faccion.Torneos = UserList(Torneo_Luchadores(LI1)).Faccion.Torneos + 1
            UserList(Torneo_Luchadores(LI1)).Ranking.TorneosGanados = UserList(Torneo_Luchadores(LI1)).Ranking.TorneosGanados + 1
            Call GuardarRanking("Torneo", Torneo_Luchadores(LI1))
            Superoro.Amount = 1
            Superoro.OBJIndex = 824
            Call MeterItemEnInventario(Torneo_Luchadores(LI1), Superoro)
    ...
    If (2 ^ Torneo_Rondas = 2 * combate) Then
        ...
        Call SendData(ToAll, 0, 0, "||Torneo: Siguiente ronda!" & FONTTYPE_GUILD)
...
Sub Rondas_UsuarioDesconecta(ByVal UserIndex As Integer)
    On Error GoTo errorh
    Call SendData(ToAll, 0, 0, "||Torneo: " & UserList(UserIndex).name & " Ha desconectado en Torneo Automatico, se le penaliza con 2.000.000 monedas de oro !!" & FONTTYPE_TALK)
...
Sub torneos_auto(ByVal rondas As Integer)
    ...
    If (Torneo_Activo) Then
        ...
        Exit Sub
    End If
    Call SendData(ToAll, 0, 0, "||Torneo: Esta empezando un nuevo torneo 1vs1 de " & val(2 ^ rondas) & " participantes!! para participar pon /ACCEDER - (No cae inventario)" & FONTTYPE_GUILD)
...
Sub Torneos_Inicia(ByVal UserIndex As Integer, ByVal rondas As Integer)
    ...
    If (Torneo_Activo) Then
        ...
        Exit Sub
    End If
    Call SendData(ToAll, 0, 0, "||Torneo: Esta empezando un nuevo torneo 1v1 de " & val(2 ^ rondas) & " participantes!! para participar pon /ACCEDER - (No cae inventario)" & FONTTYPE_GUILD)
    Call SendData(ToAll, 0, 0, "TW48")
    ...
    For i = LBound(Torneo_Luchadores) To UBound(Torneo_Luchadores)
        If (Torneo_Luchadores(i) = -1) Then
            ...
            If (i = UBound(Torneo_Luchadores)) Then
                Call SendData(ToAll, 0, 0, "||Torneo: Empieza el torneo!" & FONTTYPE_GUILD)
                frmMain.Cancelaaut.Enabled = False
                Torneo_Esperando = False
                Call Rondas_Combate(1)
...
Sub Rondas_Combate(combate As Integer)
    ...
    If (UI1 = -1) Then
        ...
        If (Torneo_Rondas = 1) Then
            If (UI2 <> -1) Then
                Call SendData(ToAll, 0, 0, "||Torneo: Torneo terminado. Ganador del torneo por eliminacion: " & UserList(UI2).name & FONTTYPE_GUILD)
                UserList(UI2).flags.automatico = False
                ' dale_recompensa()
                Torneo_Activo = False
                Exit Sub
            End If
            Call SendData(ToAll, 0, 0, "||Torneo: Torneo terminado. No hay ganador porque todos se fueron :(" & FONTTYPE_GUILD)
            Exit Sub
        End If
        If (UI2 <> -1) Then _
           Call SendData(ToAll, 0, 0, "||Torneo: " & UserList(UI2).name & " pasa a la siguiente ronda!" & FONTTYPE_TALK)
    ...
    Call SendData(ToAll, 0, 0, "||Torneo: " & UserList(UI1).name & " vs " & UserList(UI2).name & ". Esquinas!! Peleen!" & FONTTYPE_GUILD)
