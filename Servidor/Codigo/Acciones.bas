...
'but WITHOUT ANY WARRANTY; withobanut even the implied warranty of
...
Sub Accion(UserIndex As Integer, Map As Integer, X As Integer, Y As Integer)
    ...
    If MapData(Map, X, Y).NpcIndex Then
        ...
        If Npclist(MapData(Map, X, Y).NpcIndex).NPCtype = NPCTYPE_BANQUERO Then
            ...
            Exit Sub
        End If
        If Npclist(MapData(Map, X, Y).NpcIndex).NPCtype = NPCTYPE_VIAJERO Then
            Call IniciarViaje(UserIndex)
            Exit Sub
        End If
        If Npclist(MapData(Map, X, Y).NpcIndex).NPCtype = NPCTYPE_NPCQUEST Then
            SendQuest UserIndex
            Exit Sub
        End If

        If Npclist(MapData(Map, X, Y).NpcIndex).NPCtype = NPCTYPE_CONSTRUCTOR Then
            Call IniciarConstruccion(UserIndex)
            Exit Sub
        End If
        ...
        If ObjData(MapData(Map, X, Y).OBJInfo.OBJIndex).Llave = 0 Then
            If ObjData(MapData(Map, X, Y).OBJInfo.OBJIndex).Cerrada Then
                ...
                If ObjData(MapData(Map, X, Y).OBJInfo.OBJIndex).Llave = 0 Then
                    ...
                    SendData ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & SND_PUERTA & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y
