...
Public Sub EventoSockAccept(SockID As Long)
    #If UsarQueSocket = 1 Then
        ...
        If NewIndex <= MaxUsers Then
            ...
            'ANTI TIRARA DE LOGIN
            Hay_Socket = -1
            For i = 1 To MAX_CONEX
                If Anti_DDOS(i).ip = UserList(NewIndex).ip Then
                    Hay_Socket = i
                    Exit For
                End If
            Next i

            If Hay_Socket <> -1 Then
                Anti_DDOS(Hay_Socket).Conectadas = Anti_DDOS(Hay_Socket).Conectadas + 1
                If Anti_DDOS(Hay_Socket).Conectadas >= 12 Then    ' 12 conex maximas

                    BanIps.Add UserList(NewIndex).ip    'lo baneo por forro

                    If FileExist(App.path & "\BanIPs.txt", vbNormal) Then
                        Dim IpBaneadas As Integer
                        IpBaneadas = val(GetVar(App.path & "\BanIPs.txt", "INIT", "Cantidad"))
                        Call WriteVar(App.path & "\BanIPs.txt", "INIT", "Cantidad", IpBaneadas + 1)
                        Call WriteVar(App.path & "\BanIPs.txt", "BANS", "IP" & IpBaneadas + 1, UserList(NewIndex).ip)
                    Else
                        Call WriteVar(App.path & "\BanIPs.txt", "INIT", "Cantidad", 1)
                        Call WriteVar(App.path & "\BanIPs.txt", "BANS", "IP1", UserList(NewIndex).ip)
                    End If

                    Call SendData(ToAdmins, 0, 0, "||ANTI DOSS> IP: " & Anti_DDOS(Hay_Socket).ip & " (Reset Sock + BANIP)." & FONTTYPE_FIGHT)
                    For i = 1 To MaxUsers    ' reseteo los SLOTS
                        If UserList(i).ConnID <> -1 And Not UserList(i).flags.UserLogged Then Call CloseSocket(i)
                    Next

                End If
            Else
                Socket_LAST = Socket_LAST + 1
                Anti_DDOS(Socket_LAST).ip = UserList(NewIndex).ip
            End If
            'ANTI TIRADA DE LOGIN

            If aDos.MaxConexiones(UserList(NewIndex).ip) Then    '
                UserList(NewIndex).ConnID = -1
                'BanIps.Add UserList(NewIndex).ip
                'Call SendData(ToAdmins, 0, 0, "||BANIP AUTOMÁTICO> IP: " & UserList(NewIndex).ip & "." & FONTTYPE_FIGHT)
                Dim rr As Integer
