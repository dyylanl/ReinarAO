...
Function TipoClase(UserIndex As Integer) As Byte
    ...
    Select Case UserList(UserIndex).Clase
        Case ASESINO, BARDO    ' Paladin Asesino Cazador
            TipoClase = 2
        Case LADRON    ' Clerigo Bardo Ladron
            TipoClase = 3
        Case MAGO, NIGROMANTE, DRUIDA    ' MAGO NIGROMANTE DRUIDA
            TipoClase = 4
        Case PALADIN, CLERIGO
            TipoClase = 5
        Case GUERRERO
            TipoClase = 6
        Case Else
            TipoClase = 1    ' Restantes, Cazador Arquero..
    End Select
