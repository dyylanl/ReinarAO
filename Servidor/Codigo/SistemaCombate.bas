...
Public Function PoderEvasion(UserIndex As Integer) As Integer
    ...
    PoderEvasion = PoderEvasion + (1.1 * Maximo(UserList(UserIndex).Stats.ELV - 12, 0))
...
Public Function UserImpactoNpc(UserIndex As Integer, NpcIndex As Integer) As Boolean
    ...
    If UserImpactoNpc Then
        ...
        End If
        Call SendData(ToPCArea, UserIndex, Npclist(NpcIndex).pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & Npclist(NpcIndex).Char.CharIndex)
    End If
...
Public Function NpcImpacto(ByVal NpcIndex As Integer, UserIndex As Integer) As Boolean
    ...
    If UserList(UserIndex).Invent.EscudoEqpObjIndex Then
        If Not NpcImpacto Then
            ...
            If Rechazo Then
                Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & SND_ESCUDO & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y)
...
Public Sub NpcAtacaUser(ByVal NpcIndex As Integer, UserIndex As Integer)
    ...
    If (Npclist(NpcIndex).pos.Map = mapa_castilloSur) And (UserList(UserIndex).GuildInfo.GuildName = CastilloSur) Then Exit Sub
    If (Npclist(NpcIndex).pos.Map = mapa_castilloNorte) And (UserList(UserIndex).GuildInfo.GuildName = CastilloNorte) Then Exit Sub
    ...
    If Npclist(NpcIndex).flags.Snd1 Then Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & Npclist(NpcIndex).flags.Snd1 & "," & Npclist(NpcIndex).pos.X & "," & Npclist(NpcIndex).pos.Y)

    If NpcImpacto(NpcIndex, UserIndex) Then
        Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & SND_IMPACTO & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y)

        'If UserList(UserIndex).flags.Navegando = 0 Then Call SendData(ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CFF" & UserList(UserIndex).Char.CharIndex & "," & PARTSANGRE & "," & 0)
    ...
    Call SubirSkill(UserIndex, Defensa, 100)
...
Public Sub NpcAtacaNpc(ByVal Atacante As Integer, ByVal Victima As Integer)
    ...
    Else: Exit Sub
    End If
    If Npclist(Atacante).flags.Snd1 Then Call SendData(ToNPCArea, Atacante, Npclist(Atacante).pos.Map, "TW" & Npclist(Atacante).flags.Snd1 & "," & Npclist(Atacante).pos.X & "," & Npclist(Atacante).pos.Y)

    If NpcImpactoNpc(Atacante, Victima) Then

        If Npclist(Victima).flags.Snd2 Then
            Call SendData(ToNPCArea, Victima, Npclist(Victima).pos.Map, "TW" & Npclist(Victima).flags.Snd2 & "," & Npclist(Victima).pos.X & "," & Npclist(Victima).pos.Y)
        Else: Call SendData(ToNPCArea, Victima, Npclist(Victima).pos.Map, "TW" & SND_IMPACTO2 & "," & Npclist(Victima).pos.X & "," & Npclist(Victima).pos.Y)
        End If

        If Npclist(Atacante).MaestroUser Then
            Call SendData(ToNPCArea, Atacante, Npclist(Atacante).pos.Map, "TW" & SND_IMPACTO & "," & Npclist(Atacante).pos.X & "," & Npclist(Atacante).pos.Y)
        Else: Call SendData(ToNPCArea, Victima, Npclist(Victima).pos.Map, "TW" & SND_IMPACTO & "," & Npclist(Victima).pos.X & "," & Npclist(Victima).pos.Y)
        End If
        Call NpcDañoNpc(Atacante, Victima)

    Else
        If Npclist(Atacante).MaestroUser Then
            Call SendData(ToNPCArea, Atacante, Npclist(Atacante).pos.Map, "TW" & SOUND_SWING & "," & Npclist(Atacante).pos.X & "," & Npclist(Atacante).pos.Y)
        Else
            Call SendData(ToNPCArea, Victima, Npclist(Victima).pos.Map, "TW" & SOUND_SWING & "," & Npclist(Victima).pos.X & "," & Npclist(Victima).pos.Y)
        End If
    End If
...
errhandler:
    If Err.Number <> 0 Then
        LogError ("Error en UsuarioAtaca - " & Err.Description)
    End If
End Sub
Public Sub UsuarioAtacaNpc(UserIndex As Integer, ByVal NpcIndex As Integer)


    If Npclist(NpcIndex).Numero = 244 And MataGuardiasEmperador <> 3 Then
...
Public Function UsuarioImpacto(ByVal AtacanteIndex As Integer, ByVal VictimaIndex As Integer) As Boolean
    ...
    If UsuarioImpacto Then
        If UserList(AtacanteIndex).Invent.WeaponEqpObjIndex Then
            ...
            Else
                Call SendData(ToPCArea, VictimaIndex, UserList(VictimaIndex).pos.Map, "CFF" & UserList(AtacanteIndex).Char.CharIndex & "," & UserList(VictimaIndex).Char.CharIndex)
...
Public Sub UsuarioAtacaUsuario(ByVal AtacanteIndex As Integer, ByVal VictimaIndex As Integer)
    ...
    If UserList(AtacanteIndex).flags.Invisible Or UserList(AtacanteIndex).flags.Oculto Then
        Call SendData(ToIndex, AtacanteIndex, 0, "||Te han descubierto atacando invisible." & FONTTYPE_TALK)
        Call SendData(ToIndex, VictimaIndex, 0, "||Descubriste a " & UserList(AtacanteIndex).name & " atacandote invisible." & FONTTYPE_TALK)
        Call QuitarInvisible(AtacanteIndex)
        Exit Sub
    End If
    ...
    If UsuarioImpacto(AtacanteIndex, VictimaIndex) Then
        ...
        Call SendData(ToPCArea, AtacanteIndex, UserList(AtacanteIndex).pos.Map, "TW" & "10" & "," & UserList(AtacanteIndex).pos.X & "," & UserList(AtacanteIndex).pos.Y)

        'If UserList(VictimaIndex).flags.Navegando = 0 Then Call SendData(ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "CFF" & UserList(VictimaIndex).Char.CharIndex & "," & PARTSANGRE & "," & 0)
...
Sub UsuarioAtacadoPorUsuario(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer)
    ...
    If UserList(AttackerIndex).pos.Map = 190 Then Exit Sub

    Call AllMascotasAtacanUser(AttackerIndex, VictimIndex)
    Call AllMascotasAtacanUser(VictimIndex, AttackerIndex)
    If UserList(AttackerIndex).flags.EstanDueleando = True And UserList(VictimIndex).flags.EstanDueleando = True Then
        Exit Sub
    End If
    If UserList(AttackerIndex).flags.EstanDueleando = True And UserList(VictimIndex).flags.EstanDueleando = True Then
        Exit Sub
...
Public Function PuedeAtacar(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer) As Boolean
    ...
    If UserList(AttackerIndex).GuildInfo.Seguro And UserList(AttackerIndex).GuildInfo.GuildName = UserList(VictimIndex).GuildInfo.GuildName Then
        Call SendData(ToIndex, AttackerIndex, 0, "|| No puedes atacar a usuarios de tu clan. Para hacerlo tipea /SEGUROCLAN." & FONTTYPE_INFO)
        Exit Function
    End If
    ...
    If UserList(VictimIndex).flags.EstaDueleando = True Then
        If UserList(AttackerIndex).flags.Oponente <> VictimIndex Then
            Call SendData(ToIndex, AttackerIndex, 0, "||¡No podes atacar a un personaje en duelo! " & FONTTYPE_INFO)
            PuedeAtacar = False
            Exit Function
        End If
    End If
...
Public Function PuedeAtacarMascota(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer) As Boolean
    ...
    If EsNewbie(VictimIndex) And (EsNewbie(AttackerIndex) Or UserList(AttackerIndex).Faccion.Bando = Real) Then
        Call SendData(ToIndex, AttackerIndex, 0, "||Los miembros de la Alianza del Lhirius AO no pueden atacar mascotas de newbies." & FONTTYPE_INFO)
        Exit Function
    End If

    If EsNewbie(AttackerIndex) And UserList(VictimIndex).Faccion.Bando = Real Then
        Call SendData(ToIndex, AttackerIndex, 0, "||Los newbies no pueden atacar mascotas de la Alianza del Lhirius AO." & FONTTYPE_INFO)
        Exit Function
    End If
