...
Public Sub DoOcultarse(UserIndex As Integer)
    ...
    If CInt(RandomNumber(1, SUERTE)) <= 5 Then
        ...
        Call SendData(ToMap, 0, UserList(UserIndex).pos.Map, ("V3" & DesteEncripTE(UserList(UserIndex).Char.CharIndex & ",1")))
...
Public Sub DoNavega(UserIndex As Integer, Slot As Integer)
    ...
    Call ChangeUserCharB(ToMap, 0, UserList(UserIndex).pos.Map, UserIndex, UserList(UserIndex).Char.Body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.Heading, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.CascoAnim, UserList(UserIndex).Char.Alas)
...
Public Sub HerreroConstruirItem(UserIndex As Integer, ItemIndex As Integer, cantT As Integer)
    ...
    If PuedeConstruir(UserIndex, ItemIndex, cantT) And PuedeConstruirHerreria(ItemIndex) Then
        ...
        Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & MARTILLOHERRERO & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y)
    Else
...
Public Sub CarpinteroConstruirItem(UserIndex As Integer, ItemIndex As Integer, cantT As Integer)
    ...
    If CarpinteroTieneMateriales(UserIndex, ItemIndex, cantT) And _
       ...
       UserList(UserIndex).Invent.HerramientaEqpObjIndex = SERRUCHO_CARPINTERO Then
        ...
        Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & LABUROCARPINTERO & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y)
    End If
...
Public Sub DoTalar(UserIndex As Integer, Elfico As Boolean)
    ...
    If MapInfo(UserList(UserIndex).pos.Map).Pk = False Then
        SendData ToIndex, UserIndex, 0, "||Se dice en el mundo que las mejores maderas en su cálidad se encuentran fuera de zonas seguras." & FONTTYPE_INFO
        Exit Sub
    End If
    ...
    Call SendData(ToPCArea, CInt(UserIndex), UserList(UserIndex).pos.Map, "TW" & SOUND_TALAR & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y)
...
Public Sub DoPescar(UserIndex As Integer)
    ...
    If MapInfo(UserList(UserIndex).pos.Map).Pk = False Then
        SendData ToIndex, UserIndex, 0, "||No hay peces en zonas seguras, los mejores peces están en los mares de este mundo." & FONTTYPE_INFO
        Exit Sub
    End If
    ...
    If Not MeterItemEnInventario(UserIndex, MiObj) Then Call TirarItemAlPiso(UserList(UserIndex).pos, MiObj)
    Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & SOUND_PESCAR & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y)
...
Public Sub DoRobar(LadronIndex As Integer, VictimaIndex As Integer)
    ...
    If UserList(LadronIndex).pos.Map = 190 Or UserList(LadronIndex).pos.Map = 12 Or UserList(LadronIndex).pos.Map = 204 Or UserList(LadronIndex).pos.Map = 203 Or UserList(LadronIndex).pos.Map = 14 Then Exit Sub
...
Public Sub DoApuñalar(UserIndex As Integer, ByVal VictimNpcIndex As Integer, ByVal VictimUserIndex As Integer, ByVal Daño As Integer)
    ...
    Prob = 25 - 1.3 * UserList(UserIndex).Stats.UserSkills(Apuñalar) \ 10

    Select Case UserList(UserIndex).Clase
        Case ASESINO
            Prob = Prob - 2 - Buleano(UserList(UserIndex).Recompensas(3) = 2)
        Case BARDO
            Prob = Prob - 1 - Buleano(UserList(UserIndex).Recompensas(3) = 1)
    End Select
    ...
    If RandomNumber(1, Prob) <= 1 Then
        If VictimUserIndex Then
            ...
            Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "CFX" & "0," & UserList(VictimUserIndex).Char.CharIndex & "," & FXAPUÑALAR & "," & 0 & "," & 1)
...
Public Sub DoMineria(UserIndex As Integer, Mineral As Integer)
    ...
    If MapInfo(UserList(UserIndex).pos.Map).Pk = False Then
        SendData ToIndex, UserIndex, 0, "||Las mejores reservas mineras se encuentran fuera de zonas seguras." & FONTTYPE_INFO
        Exit Sub
    End If
    ...
    Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "TW" & SOUND_MINERO & "," & UserList(UserIndex).pos.X & "," & UserList(UserIndex).pos.Y)
