...
Public Sub DoTileEvents(UserIndex As Integer)
    ...
    If MapData(Map, X, Y).TileExit.Map = 216 Or MapData(Map, X, Y).TileExit.Map = 214 Then
        If UserList(UserIndex).GuildInfo.GuildName = "" Then
            Call SendData(ToIndex, UserIndex, 0, "||No tienes clan para ingresar a este mapa." & FONTTYPE_INFO)
            Exit Sub
        End If
    End If
    ...
    Select Case UserList(UserIndex).Stats.UserSkills(Supervivencia)
        ...
        Case Else
            If UserList(UserIndex).flags.Privilegios > 0 Then
                Info = "||" & Npclist(NpcIndex).name & " [" & Npclist(NpcIndex).Stats.MinHP & "/" & Npclist(NpcIndex).Stats.MaxHP & "] - [Numero NPC: " & Npclist(NpcIndex).Numero & "]"
    ...
    If InMapBounds(X, Y) Then
        ...
        If MapData(Map, X, Y).OBJInfo.OBJIndex Then
            ...
            If MapData(Map, X, Y).OBJInfo.Amount = 1 Then
                If UserList(UserIndex).flags.Privilegios > 0 Then
                    Call SendData(ToIndex, UserIndex, 0, "||Ves un (" & MapData(Map, X, Y).OBJInfo.OBJIndex & ") - " & ObjData(MapData(Map, X, Y).OBJInfo.OBJIndex).name & FONTTYPE_INFO)
                Else
                    Call SendData(ToIndex, UserIndex, 0, "||Ves un " & ObjData(MapData(Map, X, Y).OBJInfo.OBJIndex).name & "." & FONTTYPE_INFO)
                End If
            Else
                If UserList(UserIndex).flags.Privilegios > 0 Then

                    Call SendData(ToIndex, UserIndex, 0, "||Ves (" & MapData(Map, X, Y).OBJInfo.OBJIndex & ") - " & ObjData(MapData(Map, X, Y).OBJInfo.OBJIndex).name & " " & MapData(Map, X, Y).OBJInfo.Amount & FONTTYPE_INFO)
                Else
                    Call SendData(ToIndex, UserIndex, 0, "||Ves " & MapData(Map, X, Y).OBJInfo.Amount & " " & ObjData(MapData(Map, X, Y).OBJInfo.OBJIndex).name & "." & FONTTYPE_INFO)
                End If
            End If
        ...
        If FoundChar = 1 Then
            Call SendUserStatsBox(TempCharIndex)
            ...
            If UserList(TempCharIndex).Faccion.Bando Then
                ...
                ElseIf UserList(TempCharIndex).flags.EsConseCaos = 1 Or UserList(TempCharIndex).flags.EsConseReal = 1 Then
                    Stat = Stat & " <" & ListaBandos(UserList(TempCharIndex).Faccion.Bando) & "> <" & Titulo(TempCharIndex) & ">"
                Else
            ...
            If UserList(TempCharIndex).flags.Privilegios > 1 Then
                ...
                '    Stat = "||Ves a " & UserList(TempCharIndex).name & " <Semi Dios> [Imortal]" & Stat
            Else
            ...
            Stat = Stat & " <Muertos: " & UserList(TempCharIndex).flags.tdead & ">"
            If UserList(TempCharIndex).Faccion.Torneos >= 0 Then
                Stat = Stat & " <Torneos: " & UserList(TempCharIndex).Faccion.Torneos & ">"
            End If
            Stat = Stat & " <Canjeo: " & UserList(TempCharIndex).Faccion.Quests & ">"

            If UserList(TempCharIndex).flags.EsNoble = 1 Then
                Stat = Stat & " <NOBLE> "
            End If
            If UserList(TempCharIndex).Stats.PuntosDonador <> 0 Then
                Stat = Stat & " <USUARIO PREMIUM> "
            End If
...
Sub PasarSegundo()
    ...
    For i = 1 To LastUser
        ...
        If UserList(i).Counters.CreoTeleport = True Then  'si el usuario creo un teleport....
            ...
            If UserList(i).Counters.TimeTeleport = 5 Then    'cuando llega a 3
                ...
                Call SendData(ToIndex, i, 0, "||Abriste un portal hacia Althalos." & FONTTYPE_INFO)
                Dim ET As Obj
                ET.Amount = 1
                ET.OBJIndex = 804    'Acá se puede cambiar por su telep personalizado
