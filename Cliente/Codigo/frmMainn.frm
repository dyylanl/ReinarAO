...
Private Sub renderer_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If UsingSkill = Magia Then
        If Not Particula And (ClientSetup.bGraphics = 2) Then
        ...
        If LenB(CustomKeys.ReadableName(KeyCode)) > 0 Then
            ...
            Select Case KeyCode
                Case CustomKeys.BindedKey(eKeyType.mKeyToggleMusic)
                    If Musica = 1 Then
                        Musica = 0
                        If Extract_File(MP3, App.Path & "\RECURSOS\MP3\", CurrentMP3 & ".mp3", App.Path & "\RECURSOS\MP3\") Then
                            MP3P.mp3file = App.Path & "\RECURSOS\MP3\" & CurrentMP3 & ".mp3"
                            MP3P.playMP3
                        End If
                        Call Kill(App.Path & "\RECURSOS\MP3\*.mp3")
                        frmOpciones.PictureMusica.Picture = LoadPicture(DirGraficos & "tick1.gif")
                        Call WriteVar(App.Path & "\RECURSOS/Init/Opciones.opc", "CONFIG", "Musica", 0)
                        Exit Sub
                    Else
                        Musica = 1
                        MP3P.stopMP3
                        frmOpciones.PictureMusica.Picture = LoadPicture(DirGraficos & "tick2.gif")
                        Call WriteVar(App.Path & "\RECURSOS/Init/Opciones.opc", "CONFIG", "Musica", 1)
                        Exit Sub
                    End If    'X
