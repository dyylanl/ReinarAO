...
Public Const Formato As String * 6 = "DYLAN"
Public Const PasswordPak As Long = 620796
...
Public Enum resource_file_type
    ...
    Ambient
End Enum
Private Const GRAPHIC_PATH As String = "\RECURSOS\GRAFICOS\"
Private Const MIDI_PATH As String = "\Midi\"
Private Const MP3_PATH As String = "\RECURSOS\Mp3\"
Private Const WAV_PATH As String = "\RECURSOS\WAV\"
Private Const MAP_PATH As String = "\Mapas\"    'Standelf - Soporte .Maps
Private Const INTERFACE_PATH As String = "\Interface\"
Private Const SCRIPT_PATH As String = "\RECURSOS\Init\"
...
'*****************************************************************
    ...
    Select Case file_type
        Case Graphics
            ...
            Else
                SourceFilePath = resource_path & "\RECURSOS\Graficos." & Formato
            End If
        ...
        Case MP3
            ...
            Else
                SourceFilePath = resource_path & "\RECURSOS\MP3." & Formato
            End If
            OutputFilePath = resource_path & OUTPUT_PATH
        ...
        Case Scripts
            ...
            Else
                SourceFilePath = resource_path & "\RECURSOS\Init." & Formato
            End If
            OutputFilePath = resource_path & OUTPUT_PATH
...
'*****************************************************************
    ...
    Select Case file_type
        Case Graphics
            If UseOutputFolder Then
                SourceFilePath = resource_path & "\RECURSOS\Graficos." & Formato    ' & OUTPUT2_PATH & "Graficos." & Formato
            Else
                SourceFilePath = resource_path & "\RECURSOS\Graficos." & Formato
            End If
        ...
        Case map    'Standelf - Soporte .Map
            ...
            Else
                SourceFilePath = resource_path & "\RECURSOS\MAPS." & Formato
            End If
            OutputFilePath = App.Path & OUTPUT_PATH
    ...
    For loopc = 0 To UBound(InfoHead())
        ...
        Select Case LCase(Right(Trim(InfoHead(loopc).strFileName), 3))
            Case Is = "png"
                ...
                FileExtension = "png"
                OutputFilePath = resource_path & "\RECURSOS\Graficos." & Formato
                bmp_done = True
            Case Is = "mid"
            ...
            Case Is = "mp3"
                ...
                FileExtension = "mp3"
                OutputFilePath = resource_path & "\RECURSOS\MP3." & Formato
                mp3_done = True
            Case Is = "wav"
            ...
            Case Is = "ind"
                ...
                FileExtension = "ind"
                OutputFilePath = resource_path & "\RECURSOS\Init." & Formato
                ind_done = True
            Case Is = "dat"
    ...
    Select Case file_type
        Case Graphics
            If UseOutputFolder Then
                SourceFilePath = resource_path & OUTPUT_PATH & "Graficos." & Formato
            Else
                SourceFilePath = App.Path & "\RECURSOS\Graficos\Graficos.DYLAN"    ' & Formato
            End If
        ...
        Case MP3
            If UseOutputFolder Then
                SourceFilePath = App.Path & "\RECURSOS\MP3\MP3.DYLAN"    ' & Formato
            Else
                SourceFilePath = App.Path & "\RECURSOS\MP3\MP3.DYLAN"    ' & Formato
        ...
        Case Wav
            If UseOutputFolder Then
                SourceFilePath = resource_path & OUTPUT_PATH & "FXS." & Formato
            Else
                SourceFilePath = resource_path & "\FXS." & Formato
            End If
...
' Parra was here (;
    ...
    Select Case file_type
        ...
        Case Graphics
            SourceFilePath = App.Path & "\RECURSOS\Graficos\Graficos.DYLAN"

        Case Interface
            SourceFilePath = App.Path & "\RECURSOS\Graficos\Graficos.DYLAN"
        Case MP3
            SourceFilePath = App.Path & "\RECURSOS\MP3.DYLAN"
        Case Else
            Exit Function
    End Select
...
Public Function Get_Extract(ByVal file_type As resource_file_type, ByVal file_name As String) As String
    Extract_File file_type, App.Path & "\RECURSOS\Graficos", LCase$(file_name), App.Path & "\amdInData\"
...
On Local Error GoTo ErrHandler
    ...
    Select Case file_type
        ...
        Case Wav
            SourceFilePath = resource_path & "\FXS.DYLAN"
