...
'*****************************************************************
    ...
    For i = 1 To UBound(ParticleTexture())
        ...
        Set ParticleTexture(i) = D3DX.CreateTextureFromFileEx(d3ddevice, App.Path & "\RECURSOS\Graficos\Grh\" & "p" & i & ".png", D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_FILTER_NONE, &HFF000000, ByVal 0, ByVal 0)

    Next i

    For i = 1 To 4
        If Not SangreTexture(i) Is Nothing Then Set SangreTexture(i) = Nothing
        Set SangreTexture(i) = D3DX.CreateTextureFromFileEx(d3ddevice, App.Path & "\RECURSOS\Graficos\Grh\" & "2500" & i & ".png", D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_FILTER_NONE, &HFF000000, ByVal 0, ByVal 0)
    Next i
End Sub
...
'Reset the particle
    ...
    effect(EffectIndex).Particles(Index).ResetColor 0.4, 1, 0.4, 0.3 + (Rnd * 0.2), 0.05 + (Rnd * 0.07)
End Sub
...
'*****************************************************************
    ...
    effect(EffectIndex).ParticleCount = Particles - Particles * 0.25 * (2 - Opciones.bGraphics)           'Set the number of particles
    effect(EffectIndex).Used = True             'Enabled the effect
    effect(EffectIndex).X = X                   'Set the effect's X coordinate
    effect(EffectIndex).Y = Y                   'Set the effect's Y coordinate
    effect(EffectIndex).Gfx = Gfx               'Set the graphic
...
Private Sub Effect_PortalGroso_Update(ByVal EffectIndex As Integer)
    ...
    'If opciones.bGraphics < 2 Then Effect(EffectIndex).Used = False
